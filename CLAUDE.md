...
No tool modifies or deletes an existing message. `send_message` requires `confirm=true`. A test send is live data: it needs the owner's go-ahead under the rule below, and when allowed must be clearly marked `TESTING: ...` and go only to the owner's own number.
