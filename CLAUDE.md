...
No tool modifies or deletes an existing message. `send_message` requires `confirm=true`. A test send is live data: it is allowed only under the exception below, and when allowed must be clearly marked `TESTING: ...` and go only to the owner's own number.
